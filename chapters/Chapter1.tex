...
以 BEiT \cite{beit} 为代表的掩码图像建模预训练技术与 Vision Transformer (ViT) \cite{dpt}架构的应用极大增强了模型对全局几何上下文的理解。
...
\begin{figure}[!ht]    % [htbp] 表示建议 LaTeX 放置图片的位置（Here, Top, Bottom, Page）
    \centering          % 使图片居中
    \includegraphics[width=0.5\textwidth]{figures/ch1-drone.jpg} % 填入图片文件名（不带后缀也可以），width 指定宽度
    \caption{无人机} % 图片下方的标题
...
\begin{figure}[htbp]
    \centering % 【关键1】让这四个紧凑的子图整体在页面居中

    % --- 第一行 ---
    \begin{subfigure}[b]{0.35\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/ch1-rgb.jpg}
        \caption{RGB Input Image}
        \label{subfig:rgb}
    \end{subfigure}
    % 【关键2】去掉 \hfill，使用固定间距 (如 1cm 或 2em)
    \hspace{1cm}
    \begin{subfigure}[b]{0.35\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/ch1-vis_met.png}
        \caption{Metric Depth Prediction} % 标题稍微精简以防换行
        \label{subfig:met}
    \end{subfigure}

    % --- 垂直间距 ---
    \par\vspace{1em} % 强制换行并增加垂直距离

    % --- 第二行 ---
    \begin{subfigure}[b]{0.35\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/ch1-_vis_rel.png}
        \caption{Relative Depth Prediction}
        \label{subfig:rel}
    \end{subfigure}
    % 【关键3】保持和第一行一样的水平间距
    \hspace{1cm}
    \begin{subfigure}[b]{0.35\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/ch1-_vis_dis.png}
        \caption{Disparity Map}
        \label{subfig:disp}
    \end{subfigure}

    % 总标题
    \caption{Qualitative comparison of depth estimation results. (a) Input RGB image. (b) Absolute metric depth. (c) Relative depth. (d) Disparity map (inverse depth).}
    \label{fig:depth_comparison_4grid}
\end{figure}
...
为克服传统卷积神经网络（CNN）在感受野局限性与长距离依赖建模上的不足，基于Transformer\cite{vaswani2017attention}的架构一度成为单目深度估计领域的主流骨干。
Ranftl等人提出的 DPT (Dense Prediction Transformers) \cite{dpt} 
率先将Vision Transformer引入密集预测任务，利用自注意力机制捕捉全局上下文信息，显著提升了深度图的整体结构一致性。
针对深度值的离散化回归难题，AdaBins \cite{bhat2021adabins} 创新性地设计了基于Transformer的区间划分模块，
通过自适应预测深度中心来实现更精细的深度估计。
